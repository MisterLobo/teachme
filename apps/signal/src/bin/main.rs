...
use qdrant_client::{Qdrant, qdrant::{CreateCollectionBuilder, VectorParams, VectorParamsBuilder, VectorsConfigBuilder}};
use signal::app::App;
use dotenv::dotenv;
...
async fn main() -> loco_rs::Result<()> {
    dotenv().ok();
    let client = Qdrant::from_url(&env::var("QDRANT_API_URL").unwrap_or_default()).skip_compatibility_check().build().expect("failed to initialize connection");
    // client.config.set_api_key("");
